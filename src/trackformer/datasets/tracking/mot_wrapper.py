...
from .mot17_sequence import MOT17Sequence, MOT17SequenceKinet
...
class MOT17KinetWrapper(MOT17Wrapper):
    def __init__(self, split: str, det_encoder: DetectionsEncoderSine, dets: str, **kwargs) -> None:
        """Initliazes all subset of the dataset for kinematic labels.
        ...
        for seq in sequences:
            if dets == 'ALL':
                self._data.append(MOT17SequenceKinet(root_dir='data', det_transform=det_encoder,seq_name=seq,
                                                     dets='DPM', **kwargs))
                self._data.append(MOT17SequenceKinet(root_dir='data', det_transform=det_encoder,seq_name=seq,
                                                     dets='FRCNN', **kwargs))
                self._data.append(MOT17SequenceKinet(root_dir='data', det_transform=det_encoder, seq_name=seq,
                ...
                self._data.append(MOT17SequenceKinet(seq_name=seq, det_transform=det_encoder,dets=dets, **kwargs))
